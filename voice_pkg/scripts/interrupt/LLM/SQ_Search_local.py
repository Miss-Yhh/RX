...
class Searchscr:
	def __init__(self) -> None:
		...
		self.tokenizer = AutoTokenizer.from_pretrained('/home/robot/.cache/huggingface/hub/models--infgrad--stella-large-zh-v3-1792d')
		self.model = AutoModel.from_pretrained("/home/robot/.cache/huggingface/hub/models--infgrad--stella-large-zh-v3-1792d")
		self.model.eval()
		self.get_jsons()
#		self.bgeKeywords = self.get_bge_embedding()
	...
	def get_query_embedding(self,encoded_input):
	###print("tokenize完毕\n开始embedding\n")
	# Compute token embeddings
		with torch.no_grad():
			model_output = self.model(**encoded_input)
	...
	def get_bge_embedding_2(self):
		...
		# Compute token embeddings
		try:
			sentence_embeddings=torch.load('/home/kuavo/catkin_dt/src/voice_pkg/scripts/interrupt/LLM/sentence_embeddings.pt')
		except:
			with torch.no_grad():
				model_output = self.model(**encoded_input)
				# Perform pooling. In this case, cls pooling.
				sentence_embeddings = model_output[0][:, 0]
			# # normalize embeddings
			torch.save(sentence_embeddings, '/home/kuavo/catkin_dt/src/voice_pkg/scripts/interrupt/LLM/sentence_embeddings.pt')
		# print('load_done')
		sentence_embeddings=torch.load('/home/kuavo/catkin_dt/src/voice_pkg/scripts/interrupt/LLM/sentence_embeddings.pt')
		###print("embedding完毕\n")
		key_embeddings = torch.nn.functional.normalize(sentence_embeddings, p=2, dim=1)
	...
	def get_bge_query_2(self, query='说说机器人是什么'):
		...
		sentence_embeddings = self.get_query_embedding(encoded_input)
		print('************************')
...
#   print(cos_sim)
	f = Searchscr()
	print(f.get_bge_query_2('说说机器人是什么'))